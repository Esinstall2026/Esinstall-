...
function WarrantyPage({jobs,installations,warranties,onSave,onOpenJob}:{jobs:Job[];installations:Installation[];warranties:WarrantyCase[];onSave:(w:WarrantyCase)=>void;onOpenJob:(j:Job)=>void}){
 const eligible=jobs.filter(j=>installations.some(i=>i.jobId===j.id&&i.status==="Completed")||j.status==="Warranty"||warranties.some(w=>w.jobId===j.id)); const create=(jobId:string)=>onSave({jobId,openedAt:new Date().toISOString(),issue:"Installation issue",status:"Open",priority:"Medium",notes:""});
