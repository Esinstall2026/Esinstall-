...
import NewWarrantyPage from "./NewWarrantyPage";
...
type Evidence = { name: string; type: string; size: number; addedAt: string; dataUrl?: string };
...
export default function App() {
  ...
  return <div className="app-shell">
    ...
    <main className="main">
      ...
      {page === "Reports" && <ReportsModule jobs={jobs} installations={installations} warranties={warranties} production={production} />}
      {page === "Teams" && <TeamsModule jobs={jobs} installations={installations} warranties={warranties} production={production} />}
      {page === "Builders" && <BuildersModule jobs={jobs} installations={installations} warranties={warranties} production={production} />}
      {page === "Communities" && <CommunitiesModule jobs={jobs} installations={installations} warranties={warranties} production={production} />}
      {page === "Settings" && <SettingsModule jobs={jobs} installations={installations} warranties={warranties} production={production} />}
    </main>
  </div>;
}
...
function Dashboard({ jobs, installations, warranties, production, onOpenJob }: { jobs: Job[]; installations: Installation[]; warranties: WarrantyCase[]; production: ProductionRecord[]; onOpenJob: (j: Job) => void }) {
  ...
  const agendaCounts = weekDays.map((_, index) => {
    const day = new Date(weekStart);
    day.setDate(weekStart.getDate() + index);
    const iso = day.toISOString().slice(0, 10);
    return installations.filter(i => i.date === iso).length;
  });
  return <div className="content">
    <section className="hero"><div><span className="gold-label">OPERATIONS OVERVIEW</span><h2>Good morning, Administrator.</h2><p>Monitor jobs, production, installations and warranties from one place.</p></div><button className="primary" onClick={() => orderedJobs[0] && onOpenJob(orderedJobs[0])}>Open latest Job</button></section>
    <section className="metric-grid"><Metric label="Jobs in Progress" value={inProgress} note="production + installation"/><Metric label="Installations Today" value={installsToday} note="scheduled for today"/><Metric label="Open Warranties" value={warranties.filter(w => w.status !== "Resolved").length} note="needs action"/><Metric label="Teams in Field" value={teamsInField} note={`${production.length} production records`}/></section>
    <div className="two-column"><section className="panel"><PanelTitle label="OPERATIONS" title="Active Jobs"/><div className="job-list">{orderedJobs.slice(0, 5).map(j => <button className="job-row" key={j.id} onClick={() => onOpenJob(j)}><div className="job-code">{j.id}</div><div className="job-main"><strong>{j.address}</strong><span>{j.builder} · {j.community}</span></div><Status status={j.status}/><span className="arrow">→</span></button>)}</div></section><section className="panel"><PanelTitle label="WEEK" title="Installation Agenda"/><div className="agenda">{weekDays.map((d,i)=><div className="agenda-day" key={d}><span>{d}</span><strong>{agendaCounts[i]}</strong><small>installs</small></div>)}</div></section></div>
  </div>;
}
...
function JobsPage({ jobs, search, setSearch, onOpenJob }: { jobs: Job[]; search: string; setSearch: (v: string) => void; onOpenJob: (j: Job) => void }) {
  const rows = [...jobs].sort((a, b) => `${b.date}-${b.id}`.localeCompare(`${a.date}-${a.id}`)).filter(j => `${j.id} ${j.address} ${j.builder} ${j.community} ${j.team}`.toLowerCase().includes(search.toLowerCase()));
...
function ProductionPage({jobs,records,onSave,onOpenJob}:{jobs:Job[];records:ProductionRecord[];onSave:(p:ProductionRecord)=>void;onOpenJob:(j:Job)=>void}){
 const [filter,setFilter]=useState(""); const rows=records.filter(p=>{const j=jobs.find(x=>x.id===p.jobId);return j&&`${p.jobId} ${j.address} ${j.builder} ${p.material} ${p.status}`.toLowerCase().includes(filter.toLowerCase())}); const count=(s:ProductionStatus)=>records.filter(p=>p.status===s).length;
...
function WarrantyPage({jobs,installations,warranties,onSave,onOpenJob}:{jobs:Job[];installations:Installation[];warranties:WarrantyCase[];onSave:(w:WarrantyCase)=>void|Promise<void>;onOpenJob:(j:Job)=>void}){
