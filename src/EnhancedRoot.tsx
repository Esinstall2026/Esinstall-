...
export default function EnhancedRoot() {
  ...
  useEffect(() => {
    hydratePilotData().finally(() => setBooted(true));
  }, []);
  ...
  useEffect(() => {
    ...
    return () => document.removeEventListener("click", openNewWarranty, true);
  }, [booted]);
  ...
  const createJob = (job: Job) => {
    ...
    if (!installations.some(item => item.jobId === job.id)) {
      write("es-install-installations-v1", [...installations, {
        jobId: job.id,
        team: job.team,
        date: job.date,
        time: "8:00 AM",
  ...
  const createWarranty = async (value: WarrantyCase) => {
    const backend = createBackendProvider();
    const warranties = read<WarrantyCase[]>("es-install-warranty-v1", []);
    const nextWarranties = warranties.some(item => item.jobId === value.jobId)
      ? warranties.map(item => item.jobId === value.jobId ? value : item)
      : [...warranties, value];
    const jobs = read<Job[]>("es-install-jobs-v1", seedJobs);
    const job = jobs.find(item => item.id === value.jobId);
    const nextJob = job && value.status !== "Resolved" && job.status === "Completed"
      ? { ...job, status: "Warranty" as const }
      : null;
    ...
    try {
      ...
      window.setTimeout(() => {
        const warrantyButton = Array.from(document.querySelectorAll("button"))
          .find(button => button.classList.contains("nav-item") && button.textContent?.trim().endsWith("Warranty")) as HTMLButtonElement | undefined;
    ...
    return (
      <div className="auth-shell">
        <div className="auth-card">
          <span className="gold-label">ES INSTALL</span>
          <h1>Conectando…</h1>
          <p className="muted">Sincronizando os dados centrais com o dispositivo.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <>
      ...
      {showNewJob && (
        <div style={overlayStyle} role="dialog" aria-modal="true" aria-label="Create new job">
      ...
      {showNewWarranty && (
        <div style={overlayStyle} role="dialog" aria-modal="true" aria-label="Create new warranty">
          <div style={modalStyle}>
