...
export default function EnhancedRoot() {
  ...
  useEffect(() => {
    ...
    const handleGlobalClick = (event: MouseEvent) => {
      ...
      if (label === "+ New Warranty") { event.preventDefault(); event.stopPropagation(); const jobs = read<Job[]>("es-install-jobs-v1", seedJobs); const installations = read<Installation[]>("es-install-installations-v1", []); const warranties = read<WarrantyCase[]>("es-install-warranty-v1", []); const eligible = jobs.filter(job => installations.some(item => item.jobId === job.id && item.status === "Completed") || job.status === "Warranty" || warranties.some(item => item.jobId === job.id)); setWarrantyJobId(eligible[0]?.id ?? jobs[0]?.id ?? ""); setShowNewWarranty(true); }
    };
